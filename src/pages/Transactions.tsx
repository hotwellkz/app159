...
export const Transactions: React.FC = () => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (targetCategory) {
        // Проверяем, является ли источник складом, а цель - общими расходами
        if (sourceCategory.row === 4 && sourceCategory.title === 'Склад' && 
            targetCategory.row === 4 && targetCategory.title === 'Общ Расх') {
